...
class twinkle(base):
    ...
    def clock_tick(self):
        ...
        for matrix in self.marquee.matrices:
            matrix.update(
                f"{(x-int(j*self.marquee.panel_width)):03d}{(y-(i*self.marquee.panel_height)):03d}", 
                (bytearray(random.randbytes(1)), bytearray(random.randbytes(1)), bytearray(random.randbytes(1)))
            )
